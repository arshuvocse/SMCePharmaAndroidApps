package com.creatrix.salessolution.WebView;
...
import com.creatrix.salessolution.databinding.ActivitySixbyTenReportBinding;

import java.util.HashMap;
import java.util.Objects;

public class SixbyTenReportActivity extends AppCompatActivity {


    ActivitySixbyTenReportBinding binding;
    SessionManagement session;
    ProgressDialog pd;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivitySixbyTenReportBinding.inflate(getLayoutInflater());
        ...
        pd=new ProgressDialog(SixbyTenReportActivity.this);
        pd.setTitle("Loading");
        pd.setMessage("Please wait...");

        if (NetworkInformation.isConnected(SixbyTenReportActivity.this)) {
            viewLoad(empId);
        } else {
    ...
    private void viewLoad(int userId) {
        String urlBuilder = "http://103.244.247.179:181/SixTenProjectReportApps.aspx?EmpInfoId="+userId;
