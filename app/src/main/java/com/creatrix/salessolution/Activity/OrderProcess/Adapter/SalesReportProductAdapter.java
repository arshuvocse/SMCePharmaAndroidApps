...
public class SalesReportProductAdapter extends RecyclerView.Adapter<SalesReportProductAdapter.dvh> {
    private Context context;
    private Activity activity;
    private DecimalFormat df3 = new DecimalFormat("#.###");
    private final static int FADE_DURATION = 500; //FADE_DURATION in milliseconds
    private List<PWSData> aOrderList;
    public SalesReportProductAdapter(List<PWSData> aOrderList, Context context) {
    ...
    public dvh onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.rv_productwise_order, parent, false);
        context = parent.getContext();
        return new dvh(itemView);
    }
    @Override
    public void onBindViewHolder(dvh holder, int position) {
        PWSData ad = aOrderList.get(position);
        holder.rv_sn.setText(ad.getSN());
        holder.rv_name.setText(ad.getProduct_name());
        holder.rv_ot.setText(ad.getOrder_times());
