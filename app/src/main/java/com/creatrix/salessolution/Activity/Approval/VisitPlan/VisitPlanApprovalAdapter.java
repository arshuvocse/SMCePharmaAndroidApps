...
import com.creatrix.salessolution.Activity.Approval.VisitPlan.Model.AdoctorVisitplnDetails;
...
public class VisitPlanApprovalAdapter extends RecyclerView.Adapter<VisitPlanApprovalAdapter.BookViewHolder> /*implements ITourplan.View */ {
    private final static int FADE_DURATION = 500; //FADE_DURATION in milliseconds
    Rcv_TourPlanListener mListener;
    private VisitPlanApprovalData tpdata;
    private ATourPlanMaster tpM;
    private ATourPlanDate tpdate;
    private ATourPlanDtlsDAO tpdtl;
    List<ATourPlanDate> tpListdata;
    List<AdoctorVisitplnDetails> vpListdata;
    Activity activity;
   ...
   /* public TourPlanApprovalAdapter(TourPlanApprovalData tpdata, Activity activity, Rcv_TourPlanListener mListener) {
    ...
    public VisitPlanApprovalAdapter(List<ATourPlanDate> tpListdata, Activity activity) {
        this.tpListdata = tpListdata;
        this.activity = activity;
        this.mListener = mListener;

    }
    public VisitPlanApprovalAdapter(List<AdoctorVisitplnDetails> vpListdata, Activity activity, Rcv_TourPlanListener mListener) {
        this.vpListdata = vpListdata;
        this.activity = activity;
        this.mListener = mListener;
    ...
    public class BookViewHolder extends RecyclerView.ViewHolder {
        public TextView dateName, tv_day, tv_date,docName;
    ...
    public void onBindViewHolder(BookViewHolder holder, int position) {
        AdoctorVisitplnDetails data = vpListdata.get(position);
        holder.dateName.setText(data.getDayValue() + "\n" + data.getDayName());
        holder.docName.setVisibility(View.VISIBLE);
        holder.docName.setText(data.getaVisitDtlsDAO().get(0).getDoctorName());

        //nested recycler
       /* if (data.getaVisitDtlsDAO().size() > 0) {
            _TourInfoData_List dpter;

            dpter = new _TourInfoData_List(data.getaVisitDtlsDAO().get(0).getDoctorName(), activity, mListener);
    ...
    public int getItemCount() {
        if(vpListdata.size()>0)
        {
            return vpListdata.size();
        }else return 0;
