...
public class ReceiveableReportActivity extends AppCompatActivity {
     ActivityReceiveableReportBinding binding;
    SessionManagement session;
    ProgressDialog pd;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityReceiveableReportBinding.inflate(getLayoutInflater());
        ...
        pd=new ProgressDialog(ReceiveableReportActivity.this);
        pd.setTitle("Loading");
        pd.setMessage("Please wait...");

        if (NetworkInformation.isConnected(ReceiveableReportActivity.this)) {
            viewLoad(empId);
        } else {
    ...
    private void viewLoad(int userId) {
        String urlBuilder = "http://103.244.247.179:181/SInventory_UI/NewReceiveableReportApps.aspx?EMPID="+userId;
