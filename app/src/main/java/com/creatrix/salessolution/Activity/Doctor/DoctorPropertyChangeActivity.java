...
import com.creatrix.salessolution.databinding.ActivityDoctorPropertyChangeBinding;
...
public class DoctorPropertyChangeActivity extends AppCompatActivity implements ICustomerAdd.View, IMarketStracture.View {
ActivityDoctorPropertyChangeBinding binding;
    ProgressDialog pd;
    SessionManagement session;
    int empId, selectedGroupId, selectedRegionId, selectedAreaId, selectedTeriId, selectedSTeri, selectedMarket;
    String Role;
    DoctorARModel doctorData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding=ActivityDoctorPropertyChangeBinding.inflate(getLayoutInflater());
        //setContentView(R.layout.activity_doctor_property_change);
        setContentView(binding.getRoot());
        binding.toolbarCustom.setNavigationOnClickListener(v -> finish());
        session = new SessionManagement(DoctorPropertyChangeActivity.this);
        ...
        presenter = new CustomerPresenter(this, DoctorPropertyChangeActivity.this);
        mkpresenter = new MarketStructurePresenter(this, DoctorPropertyChangeActivity.this);
        presenter.GetProgramType();

        Gson gson = new Gson();
        doctorData = gson.fromJson(getIntent().getStringExtra("DoctorData"), DoctorARModel.class);
        initAllMaketLoad();
        System.out.println("data Cust Property : " + doctorData);
        pd = new ProgressDialog(DoctorPropertyChangeActivity.this);
        ...
        binding.updateProvider.setOnClickListener(view ->
        {
            ...
            postProvider.put("DoctorId", String.valueOf(doctorData.getDoctorId()));
        ...
        binding.updateMarket.setOnClickListener(view -> {
            UpdateDocMarket updmarket = new UpdateDocMarket();
            ...
            int territoryzId=teri.getTerritoryId();
            ...
            updmarket.setDoctorId(String.valueOf(doctorData.getDoctorId()));
            updmarket.setEmpId(String.valueOf(empId));
            updmarket.setGroupId(grpId);
            updmarket.setZoneId(regionId);
            updmarket.setAreaId(areaId);
            updmarket.setTeritoryId(territoryzId);
            updmarket.setSTeritoryId(sterritoryId);
            updmarket.setMarketId(marketId);
            updateMarket(updmarket);
        });
    }
    private void initAllMaketLoad() {
        ...
        mkpresenter.GetTeritoryLocal(Integer.parseInt(doctorData.getAreaId()));
        edit_mName=doctorData.getTerritoryName();
    ...
    private void setData(DoctorARModel doctorData) {
        if (doctorData.getDoctorTypeName() != null) {
            String edit_dName = doctorData.getDoctorTypeName();
            binding.providerSpinner.setSelection(getIndex(binding.providerSpinner, edit_dName));
        }
    }
    private void updateMarket(UpdateDocMarket upmarket) {
        pd.show();
        try {
            ApiCustomerCall service = RetrofitClientInstance.getRetrofitInstance().create(ApiCustomerCall.class);
            Call<ResultInfo> call = service.UpdateDoctorMarket(upmarket);
            call.enqueue(new Callback<ResultInfo>() {
                ...
                public void onResponse(@NonNull Call<ResultInfo> call, @NonNull Response<ResultInfo> response) {
                    ...
                    if (response.body().isSuccess) {
                        ...
                        new AlertDialog.Builder(DoctorPropertyChangeActivity.this)
                                .setTitle("Success")
    ...
    private void updateProvider(HashMap<String, String> postProvider) {
        ...
        try {
            ApiCustomerCall service = RetrofitClientInstance.getRetrofitInstance().create(ApiCustomerCall.class);
            Call<ResultInfo> call = service.UpdateDoctorProviderType(postProvider);
            call.enqueue(new Callback<ResultInfo>() {
                @Override
                public void onResponse(@NonNull Call<ResultInfo> call, @NonNull Response<ResultInfo> response) {
                    if (pd != null || pd.isShowing()) {
    ...
    public void onProgramType(List<ProgramType> ptype) {
        try {
            ArrayAdapter<ProgramType> dataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, ptype);// dbDoctor.getProgramTypeListFromSQLite(0));
    ...
    public void vGroup(List<Group> groupList) {
        try {
            if (groupList != null) {
                gdataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, groupList);
    ...
    public void vRegion(List<Region> regionList) {
        try {
            if (regionList != null) {
                rdataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, regionList);
    ...
    public void vArea(List<Area> areaList) {
        try {
            if (areaList != null) {
                adataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, areaList);
    ...
    public void vTeritory(List<Teritorry> teritoryList) {
        try {
            if (teritoryList != null) {
                tdataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, teritoryList);
    ...
    public void vSTeritory(List<SubTeritorry> steritoryList) {
        try {
            if (steritoryList != null) {
                stdataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, steritoryList);
                ...
                edit_mName=doctorData.getSubTerritoryName();
    ...
    public void vMarket(List<Market> marketList) {
        try {
            if (marketList != null) {
                mdataAdapter = new ArrayAdapter<>(DoctorPropertyChangeActivity.this, android.R.layout.simple_spinner_item, marketList);
